...
test('Phase 11 version-1 fixture migrates into a version-2 envelope without outcome loss', async () => {
    ...
    assert.equal(envelope.version, 11);
    ...
    assert.equal(envelope.player.version, 1);
    assert.equal(envelope.player.location, 'ship');
    assert.equal(envelope.player.encounterId, null);
    assert.equal(envelope.ship.version, 2);
...
test('validated import creates a new slot and rejects changed, corrupt, and future data safely', () => {
    ...
    assert.throws(() => slots.previewImport(future), /newer than supported version 11/);
